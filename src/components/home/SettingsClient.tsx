...
const STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  ...
  expired:   { label: "已到期",   className: "text-muted-foreground/60 bg-muted/50 border-border" },
    ...
    <div ref={pageRef} className="h-full overflow-y-auto scrollbar-none">
      <div className="max-w-lg mx-auto px-4 sm:px-6 py-7 pb-16 space-y-4">
        ...
        <div className="settings-card opacity-0 mb-6">
          <h1 className="text-xl font-bold text-foreground">账户设置</h1>
          <p className="text-muted-foreground text-sm mt-1">管理个人信息与订阅</p>
        </div>

        {/* Avatar + Profile */}
        <div
          className="settings-card opacity-0 rounded-2xl border border-border p-6"
          style={{ background: "var(--surface)" }}
        >
          {/* Avatar upload */}
          <div className="flex flex-col items-center gap-3 mb-6">
            ...
            >
              <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-border ring-4 ring-violet-500/15">
                {currentAvatar ? (
              ...
              {/* Preview indicator */}
              {avatarPreview && (
                <div className="absolute -bottom-0.5 -right-0.5 w-6 h-6 rounded-full bg-violet-500 border-2 border-background flex items-center justify-center">
            ...
            <div className="text-center">
              <p className="text-[12px] text-muted-foreground">点击更换头像</p>
              <p className="text-[10px] text-foreground/40 mt-0.5">JPG / PNG / WebP · 最大 1.5MB</p>
            </div>
            {avatarPreview && (
              <button
          ...
          <div className="space-y-2">
            <label className="text-[11px] text-muted-foreground font-medium uppercase tracking-wider block">
              昵称
            </label>
            <input
              value={name}
              ...
              maxLength={50}
              placeholder="输入昵称"
              className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/30 transition-all"
            />
          </div>
        ...
        <SectionCard title="账号安全">
          ...
          <div className="flex items-center justify-between py-0.5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] text-muted-foreground">手机号</p>
                <p className="text-sm text-foreground/70 font-medium mt-0.5">{maskPhone(initialUser.phone)}</p>
              </div>
            </div>
            {initialUser.phone ? (
          ...
          {showPhoneBind && (
            <div className="space-y-3 pt-1 pb-2">
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground shrink-0">+86</span>
                <input
                  value={phoneValue}
                  onChange={(e) => setPhoneValue(e.target.value.replace(/\D/g, ""))}
                  maxLength={11}
                  placeholder="输入手机号"
                  className="flex-1 bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-violet-500/50 transition-all"
                />
              </div>
              <div className="flex items-center gap-2">
                <input
          ...
          <div className="flex items-center justify-between py-0.5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] text-muted-foreground">微信账号</p>
                <p className="text-sm text-foreground/70 font-medium mt-0.5">{initialUser.hasWechat ? "已绑定" : "未绑定"}</p>
              </div>
            </div>
        ...
        {showWechatBind && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
              className="relative rounded-2xl border border-border p-6 w-full max-w-sm bg-card"
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-foreground">绑定微信账号</h3>
                <button
        ...
        <SectionCard title="会员信息">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-foreground/70">当前套餐</p>
              ...
              {initialUser.isPartner && (
                <p className="text-[11px] text-muted-foreground mt-0.5">永久有效</p>
              )}
            </div>
            <span
        ...
        <SectionCard title="订阅记录">
          {subsError ? (
            ...
            <div className="flex flex-col items-center py-8 gap-2">
              <ReceiptText className="h-8 w-8 text-foreground/10" />
              <p className="text-sm text-muted-foreground/60">暂无订阅记录</p>
            </div>
          ) : (
            <div>
              {subs.map((sub) => (
